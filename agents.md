...
1. **Install globally:** `npm install -g @xmatics/xsana`
...
- **Local testing header** (above the copy line) — contains the `import` from `@xmatics/xsana` which provides `log`, GID variables, and all API instances. This section is NOT copied to Asana.
...
- Import everything from `@xmatics/xsana` in the local testing header
